...
public class OrdersManager {
    ...
    void printAllOrders() {
        for (String name : customersOrders.keySet()) { // цикл проходит по ключам
    ...
    void printCustomerOrders(String customerName) {
        if (customersOrders.containsKey(customerName)) { // проверка наличия ключа
    ...
    void removeUnprofitableOrders() {
        ArrayList<String> names = new ArrayList<>(); // список клиентов с заказами меньше 5000

        // наполняем список names
        for (String name : customersOrders.keySet()) {
            double ordersSum = 0;
            for (double orderPrice : customersOrders.get(name)) {
        ...
        for (String name : names) { // удаляем из хеш-таблицы тех, чьи расходы строго меньше 5000
